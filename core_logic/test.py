    ...
    try:
        ...
        def get_route_score(route):
            try:
                ...
                else: score = score_obj
                return float(score) if isinstance(score, (int, float)) else 0.0
            except Exception: return 0.0
        
        sorted_routes = sorted(routes, key=get_route_score, reverse=True)
        
        for index, route in enumerate(sorted_routes[:3]):
            route_score = get_route_score(route)
            route_info = {"id": f"route_{chr(97 + index)}", "overall_yield": route_score * 100, "steps": [], "evaluation": {}}
            route_description_for_eval = []
            
            reaction_steps = list(getattr(route, 'reaction_tree', {}).reactions() or [])
            ...
            for step_index, reaction in enumerate(reversed(reaction_steps)):
                ...
                # LLM Elaboration Logic...
                reaction_smiles_str = f"{'.'.join(reactants_smiles)}>>{product_smiles}"
                step_details = {"title": "Reaction", "conditions": "N/A", "notes": "N/A"}
                try:
                    prompt = format_prompt("elaborate_synthesis_step", reaction_smiles=reaction_smiles_str)
                    if prompt:
                        response = generate_text(prompt, temperature=0.5)
                        if response: step_details = json.loads(response.strip().replace('```json', '').replace('```', ''))
                except Exception as e:
                    logger.warning(f"LLM step elaboration failed: {e}")
                ...
                route_info["steps"].append({
                    ...
                    "yield": yield_value, # <-- Corrected value
            ...
            if route_description_for_eval:
                try:
                    prompt = format_prompt("evaluate_synthesis_route", route_description=" -> ".join(route_description_for_eval))
                    if prompt:
                        response = generate_text(prompt, temperature=0.5)
                        if response: route_info["evaluation"] = json.loads(response.strip().replace('```json', '').replace('```', ''))
                except Exception as e:
                    logger.warning(f"LLM route evaluation failed: {e}")
