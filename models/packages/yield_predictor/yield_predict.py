...
from yield_predictor.models import SmilesClassificationModel
import numpy as np
import torch
...
class YieldPredictorAPI:
    def __init__(self, model_state_path, cuda_device=-1) -> None:
        ...
        use_cuda = True if torch.mps.is_available() else False
        if cuda_device == -1:
            use_cuda = False
        ...
        self.yield_mean = 79.29119663076209   # 必须和模型配套 不要改
        self.yield_std = 18.858441890553195   # 必须和模型配套 不要改
...
if __name__ == "__main__":
    ...
    print(yield_predictor.predict(reaction))
    print(yield_predictor.predict(reaction, to_yield_class=True))