...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleSearch(event) {
        ...
        const literatureQuery = (targetMoleculeInput.value.trim() + ' ' + searchInput.value.trim()).trim();
        const smilesQuery = targetMoleculeInput.value.trim();

        // Always run literature search if there's any text
        if (literatureQuery) {
            fetchLiterature(literatureQuery);
        }

        // Only run synthesis planning if a SMILES string is provided
        if (smilesQuery) {
            fetchSynthesisPlan(smilesQuery);
        } else {
            // If no SMILES, ensure the synthesis section is hidden
    ...
    async function fetchLiterature(query) {
        literatureResultsContainer.innerHTML = '<p class="text-center text-gray-400">Searching literature...</p>';
    ...
    async function fetchSynthesisPlan(smiles) {
        // Show synthesis section and loading state
        synthesisSection.classList.remove('hidden');
        routeTabsContainer.innerHTML = '';
        routeContentContainer.innerHTML = '<p class="text-center text-gray-400 py-8">Planning synthesis routes... This may take a minute.</p>';
        ...
        try {
            ...
            let userMessage = 'Failed to plan synthesis routes.';
            
            if (error.message.includes('AiZynthFinder tree not initialized')) {
                userMessage = 'Synthesis planning service is not properly configured. Please contact the administrator.';
            } else if (error.message.includes('Invalid SMILES')) {
                userMessage = 'Invalid SMILES string provided. Please check your input.';
            ...
            routeContentContainer.innerHTML = `
                <div class="text-center py-8">
                    <p class="text-red-400 mb-2">${userMessage}</p>
                    <p class="text-gray-500 text-sm">Error details: ${error.message}</p>
                ...
                <div class="bg-gray-700 rounded-lg p-4 mb-4">
                    <div class="flex justify-between items-start">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <span class="text-xs bg-green-900 text-green-300 px-2 py-1 rounded">Citations: N/A</span>
                        </div>
                    </div>
                    <p class="text-sm text-gray-300 mb-3">${paper.abstract}</p>
    ...
    function renderRouteTabs(routes) {
        ...
        routes.forEach((route, index) => {
            const isActive = index === 0 ? 'border-blue-500 text-blue-500' : 'border-transparent text-gray-500 hover:text-gray-300 hover:border-gray-300';
            tabsHtml += `
                <a href="#" class="${isActive} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm route-tab" data-route-id="${route.id}">
                    Route ${String.fromCharCode(65 + index)} (${route.overall_yield.toFixed(0)}% yield)
                </a>
            `;
        });
        tabsHtml += '</nav>';
        ...
        document.querySelectorAll('.route-tab').forEach(tab => {
            tab.addEventListener('click', function(e) {
                ...
                document.querySelectorAll('.route-tab').forEach(t => {
                    ...
                    t.classList.add('border-transparent', 'text-gray-500', 'hover:text-gray-300', 'hover:border-gray-300');
                });
                this.classList.remove('border-transparent', 'text-gray-500', 'hover:text-gray-300', 'hover:border-gray-300');
    ...
    function renderRouteVisualization(route) {
        ...
        route.steps.forEach(step => {
            ...
            vizHtml += `
                <div class="reaction-arrow flex-shrink-0 mx-2 my-4 md:my-0"><i class="fas fa-long-arrow-alt-right"></i></div>
                <div class="molecule-display ${borderColor} flex-shrink-0">
                    <div class="text-center">
                        <div class="text-xs text-gray-400 mb-1">Step ${step.step_number}: ${step.yield.toFixed(0)}% yield</div>
        ...
        steps.forEach(step => {
            detailsHtml += `
                <div class="bg-gray-700 rounded-lg p-4">
                    ...
                    <div class="text-xs text-gray-400 mb-2"><span class="font-medium">Predicted Yield:</span> ${step.yield.toFixed(0)}%</div>
    ...
    function renderRouteEvaluation(evaluation) {
        const advantages = (evaluation.advantages || []).map(adv => `<li class="flex items-start"><i class="fas fa-check-circle text-green-500 mt-1 mr-2"></i><span>${adv}</span></li>`).join('');
        const challenges = (evaluation.challenges || []).map(chal => `<li class="flex items-start"><i class="fas fa-exclamation-triangle text-yellow-500 mt-1 mr-2"></i><span>${chal}</span></li>`).join('');
        return `
            <div class="bg-gray-700 rounded-lg p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <ul class="text-sm text-gray-400 space-y-2">${advantages}</ul>
                    </div>
                    <div>
                        <h4 class="text-sm font-medium text-gray-300 mb-2">Challenges</h4>
                        <ul class="text-sm text-gray-400 space-y-2">${challenges}</ul>
                    </div>
                </div>
            </div>
        `;
